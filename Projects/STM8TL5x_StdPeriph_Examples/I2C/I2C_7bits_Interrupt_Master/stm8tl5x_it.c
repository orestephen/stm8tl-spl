/**
  ******************************************************************************
  * @file    I2C/I2C_7bits_Interrupt_Master/stm8tl5x_it.c
...
/**
  * @brief PXS Interrupt routine.
  * @param None
  * @retval None
  */
INTERRUPT_HANDLER(PXS_IRQHandler, 2)
{
    /* In order to detect unexpected events during development,
       it is recommended to set a breakpoint on the following instruction.
    */
...
INTERRUPT_HANDLER(I2C_IRQHandler, 29)
{
  ...
  if (I2C_CheckEvent(I2C_EVENT_MASTER_MODE_SELECT) == SUCCESS)
  { 
	  /* Send address (0xA0) */
    I2C_Send7bitAddress(0xA0, I2C_Direction_Transmitter);
    temp1 = 1;
  }

	/* Test on EV6 and clear it */
  if ((I2C_CheckEvent(I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED) == SUCCESS) && (temp1 == 1))
  {
    temp1 = 2;
  ...
  if (I2C_CheckEvent(I2C_EVENT_MASTER_BYTE_TRANSMITTING) == SUCCESS)
  {
		I2C_SendData(DataToSend++);
		CountData++;
	}
